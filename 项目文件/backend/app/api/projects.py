...
async def create_project_document_endpoint(
    ...
    return UnifiedResponse(data=ProjectResponse.model_validate(doc))
...
async def list_project_documents_endpoint(
    ...
    items = [ProjectResponse.model_validate(d) for d in docs]
    return UnifiedResponse(data=ProjectListResponse(items=items, total=total))


@router.get("/{document_id}", response_model=UnifiedResponse[ProjectResponse])
...
@router.put("/{document_id}", response_model=UnifiedResponse[ProjectResponse])
